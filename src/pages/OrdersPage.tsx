...
export default function OrdersPage() {
  ...
  const [selectedPeriod, setSelectedPeriod] = useState<"all" | "3month" | "6month" | "1year">("3month");
      ...
      <div className="max-w-6xl mx-auto">
        ...
        {loading ? (
          <div className="text-center py-20">
            ...
            <h2 className="text-xl font-bold mb-2">주문 내역을 로딩 중입니다</h2>
              ...
              <div key={order.id} className="bg-white border rounded-lg overflow-hidden">
                ...
                <div className="bg-gray-50 border-b px-6 py-4">
                  <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                    <div className="flex items-center gap-4">
                      <div>
                        <p className="text-sm text-gray-600 mb-1">주문번호: {order.id}</p>
                        <p className="text-xs text-gray-500">{order.date}</p>
                      </div>
                    </div>
                    <div className={`flex items-center gap-2 px-3 py-1.5 rounded-full font-bold text-sm ${getStatusColor(order.status)}`}>
                ...
                <div className="p-6">
                  ...
                  <div className="mt-4 flex flex-col sm:flex-row gap-2">
                    ...
                    {order.trackingNumber && order.status !== "취소" && (
                      <button
                        onClick={() => alert(`배송조회 번호: ${order.trackingNumber}`)}
                        className="flex-1 bg-black text-white rounded px-4 py-2 font-bold hover:bg-gray-800 flex items-center justify-center gap-2"
                      >
                        <Search size={16} />
                        배송 조회
                      </button>
